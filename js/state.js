/* ════════════════════════════════════════════════════════════ GP DA FAMÍLIA — state.js ════════════════════════════════════════════════════════════
   Estado global centralizado da aplicação. Todos os outros módulos (render.js,
   missions.js, parent-panel.js, main.js) leem e mutam o objeto `state` exportado
   daqui — nenhum deles mantém cópia própria de dados, e nenhum deles fala com
   localStorage diretamente (isso é trabalho exclusivo de storage.js).
   ════════════════════════════════════════════════════════════ */
import {
...
export const MEMBER_COLOR_PALETTE = [
  '#378add', '#e879c9', '#5cb832', '#e8b800',
  '#cb3232', '#8a5cf6', '#38b6ce', '#f2884b'
];
...
/* ════════════════ ASSIGNEE (RESPONSÁVEIS DE UMA TAREFA) ════════════════
   `mission.assignee` pode assumir 3 formatos, de propósito, para nunca forçar
   migração destrutiva de dados antigos:
   - 'compartilhada' → todos os membros ATUAIS (dinâmico; inclui quem for cadastrado depois — comportamento antigo)
   - array de ids → tarefa dividida entre membros específicos (pode ter 1 ou vários — "feita em conjunto")
   - string de 1 id → formato legado (pré multi-atribuição)
   Todo o resto do app lê o responsável SEMPRE através desta função — nunca
   comparando `assignee` diretamente. */
...
/* ════════════════ VALORES PADRÃO (primeira execução) ════════════════
   ⚠️ CORREÇÃO: agora a configuração padrão é completamente vazia.
   Nenhum membro, nenhuma tarefa — a família começa do zero. */
function defaultConfig() {
  return {
    members: [],               // ← sem membros padrão
...
export async function loadDateContext(dateKey) {
  ...
  state.weekState = week || {
    weekKey: weekKeyOfDateKey(normalized),
    days: {},
    finalized: false
  };
}
...
export async function loadState() {
  ...
  // Auto-cura de dados legados: configurações salvas antes de existirem cor por
...
/* ════════════════ PERSISTÊNCIA DE MUDANÇAS ════════════════
   Wrappers finos: sempre persistem o `state` atual através de storage.js.
   Nenhum outro módulo grava no localStorage. */
