...
import {
  tryFinalize, finalizeDay, restartDay,
...
/* ════════════════════════════════════════════════════════════
   APROVAÇÃO POR PIN (fallback)
   ════════════════════════════════════════════════════════════
   missions.js dispara 'gp:request-pin-approve' quando
   state.config.requireApproval é true, esperando um painel dos
   pais com PIN que ainda não existe nesta primeira versão.
   Por padrão requireApproval é false e não há UI para ativá-lo,
   então este listener nunca deve disparar em uso normal — mas
   fica registrado para não deixar o evento "no vácuo" caso o
   valor seja alterado manualmente no localStorage.
   PENDÊNCIA PARA V2: implementar o painel dos pais com PIN. */
function wirePinApprovalFallback() {
